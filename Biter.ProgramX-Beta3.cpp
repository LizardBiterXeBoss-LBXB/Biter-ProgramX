...
void Calc(void) {
    ...
    std::cout << "符号: + - * / %" << std::endl;
    std::cout << "输入 0 0 退出" << std::endl;
    ...
    while (true) {
        ...
        if (a == 0 && b == 0) {
            std::cout << "Exit calculator..." << std::endl;
            Sleep(800);
            break;
        }
        ...
        case '/':
            ...
            break;
        case '%': {
            // 取模仅支持整数，将 a,b 转为 long long
            long long ia = (long long)a;
            long long ib = (long long)b;
            if (ib == 0) {
                std::cout << "Error: Mod zero invalid!" << std::endl;
                err = true;
                break;
            }
            res = (number)(ia % ib);
...
void notepad(void) {
    ...
    while (true) {
        console_clear();
        std::cout << "================== 记事本 ==================" << std::endl;
        std::cout << "1. 开新的一页" << std::endl;
        std::cout << "2. 查看已经保存的" << std::endl;
        ...
        if (opt == 0) break;
            ...
            console_clear();
            std::cout << "============ 查看已经保存的 ============" << std::endl;
            std::ifstream inFile(".\\BiterData\\NoteData.text");
            if (!inFile.is_open()) {
                std::cout << "你没有保存文件" << std::endl;
            }
            else {
                std::string line;
...
void about(void) {
    ...
    std::cout << "Biter.ProgramX-Beta3" << std::endl;
    std::cout << "开发者: ZengLizard" << std::endl;
    std::cout << "版本:" << "Beta3" << std::endl;
    std::cout << "发布日期: 2024-06-01" << std::endl;
    std::cout << "[]==============================================[]" << std::endl;
    std::cout << "\n按下Enter键退出";
    std::cin.get();
}
...
void print_logo() {
    ...
    std::cout << "#       #    #  #   #  #  #   ##" << std::endl;
    std::cout << "######  ######  #####  #  #    #" << std::endl;
    std::cout << "        LizardBiterXe CO.       " << std::endl;
}
...
void choose_login_or_register(const std::wstring& user_data_filepath) {
    while (true) {
        ...
        try {
            ...
            std::cerr << "操作失败,ERCD:" << e.what() << std::endl;
            std::cout << "按回车继续...";
            std::cin.get();
        }
    }
